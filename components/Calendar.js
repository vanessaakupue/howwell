...
export default function Calendar(props) {
  ...
  function handleIncrementMonth(val) {
    // +1 -1
    // if we hit the bounds of the month then we can just adjust the year displayed
          ...
          return (
            <div key={rowIndex} className='grid grid-cols-7 gap-1'>
              {dayList.map((dayOfWeek, dayOfWeekIndex) => {
                ...
                return (
                  <div style={{background: color}} className={'text-xs sm:text-sm border border-solid p-2 flex text-center gap-2 justify-center rounded-lg ' + (isToday ? 'bg-indigo-400 ' : 'border-indigo-100 ') + (color === 'white' ? ' text-indigo-400' : ' text-white')} key={dayOfWeekIndex}>
                    <p>{dayIndex}</p>
                  </div>
                )
              })}
